...
function updateTapePlayerAvailability(): void {
  tapeBtn.disabled = !tapeLoaded;
  if (tapeEjectBtn) tapeEjectBtn.disabled = !tapeLoaded;
}

let suppressNextTapeStoppedStatus = false;

client.onTapeStatus = (playing) => {
  tapePlaying = playing;
  updateTapeUi();
  if (!playing && suppressNextTapeStoppedStatus) {
    suppressNextTapeStoppedStatus = false;
    return;
  }
  ...
  if (storedRom) {
    ...
    if (storedMedia) {
      if (storedMedia.format === "sna" || storedMedia.format === "z80") {
        ...
        suppressNextTapeStoppedStatus = true;
...
async function renderLibrary(): Promise<void> {
  ...
  const tapes = allTapes.filter((t) => {
    const is128k = is128kOrAboveTape(t.name, t.filename, t.data, t.machine);

    if (libraryFilter === "tap" && t.format !== "tap") return false;
    if (libraryFilter === "tzx" && t.format !== "tzx") return false;
    if (libraryFilter === "48k" && is128k) return false;
    if (libraryFilter === "128k" && !is128k) return false;

    if (activeModel === "48k" && is128k && libraryFilter !== "128k") {
      return false;
    }
  ...
  if (tapes.length === 0) {
    if (allTapes.length === 0) {
      tapeLibraryList.innerHTML =
        '<div class="tape-library-empty">No tapes yet. Click + to add.</div>';
    } else {
      const is48kFiltering = activeModel === "48k" && libraryFilter !== "128k";
      const hasHidden128k =
        is48kFiltering &&
        allTapes.some((t) => is128kOrAboveTape(t.name, t.filename, t.data, t.machine));
      const message = hasHidden128k
        ? "No 48K tapes match (128K tapes are hidden)."
        : "No tapes match the filter.";
      tapeLibraryList.innerHTML = `
        <div class="tape-library-empty">
          <div>${message}</div>
          <button id="tape-library-clear-filter-btn" class="btn" type="button" style="margin-top: 0.5rem; font-size: 0.7rem; padding: 0.25rem 0.6rem;">Clear filter</button>
        </div>
      `;
      document.getElementById("tape-library-clear-filter-btn")?.addEventListener("click", () => {
        tapeLibrarySearch.value = "";
        tapeLibraryFormatFilter.value = "all";
        libraryFilterText = "";
        libraryFilter = "all";
        void renderLibrary();
      });
  ...
  for (const tape of tapes) {
    ...
    const is128k = is128kOrAboveTape(tape.name, tape.filename, tape.data, tape.machine);
    const safeName = escapeHtml(tape.name);
    const safeFilename = escapeHtml(tape.filename);
    const safeFormat = escapeHtml(tape.format);
    const machineBadge = is128k ? '<span class="tape-library-item-machine">128K</span>' : "";
...
async function onLibraryFileSelect(files: FileList | null): Promise<void> {
  ...
  for (const file of Array.from(files)) {
    ...
    const data = await file.arrayBuffer();
    const tapeName = stripExtension(file.name);
    const machine = detectTapeMachine(tapeName, file.name, data);
    await addTape({
      name: tapeName,
      filename: file.name,
      format,
  ...
  await renderLibrary();
  if (added > 0) setStatus(`Added ${added} tape(s) to library.`);
}
...
function onLibraryTapeClick(entry: TapeEntry): void {
  ...
  if (
    currentModel() === "48k" &&
    is128kOrAboveTape(entry.name, entry.filename, entry.data, entry.machine)
  ) {
    const ok = window.confirm(`"${entry.name}" requires a 128K Spectrum. Switch to 128K machine?`);
    if (!ok) return;
    void switchModel("128k").then(() => promptLoadTape(entry));
    return;
  }
  promptLoadTape(entry);
}

function promptLoadTape(entry: TapeEntry): void {
  pendingTapeEntry = entry;
  if (tapeLoaded) {
    confirmLoadName.textContent = entry.filename;
    confirmLoadModal.style.display = "flex";
  } else {
    void loadTapeIntoPlayer();
  }
}

async function loadTapeIntoPlayer(): Promise<void> {
  ...
  setStatus(`Loaded "${entry.filename}" into the player. Press Play to start.`);
}
...
async function loadMediaFile(file: File): Promise<void> {
  ...
  if (snapshotExt) {
    ...
    setStatus(`Unrecognized file type: "${file.name}" (expected .sna/.z80/.dsk)`, "warn");
    return;
  }
...
document.body.addEventListener("drop", async (e) => {
  ...
  } else if (Object.keys(TAPE_EXTENSIONS).some((ext) => name.endsWith(ext))) {
    await onLibraryFileSelect(files);
    setStatus("Added tape(s) to library. Click it in the library to play.");
  } else {
  ...
  if (!document.fullscreenEnabled) {
    ...
    fullscreenBtn.addEventListener("click", () => {
      if (document.fullscreenElement) {
        ...
        void screenFrame.requestFullscreen();
      }
    });
...
async function instantLoadCurrentTape(): Promise<void> {
  client.stopTape();
  client.reset(currentModel() === "128k");
  await ensureAudioStarted();
  client.playTape();
  await sleep(2500);
  await typeText('j""\n');
...
tapeBtn.addEventListener("click", () => {
  if (tapePlaying) {
    client.stopTape();
  } else if (fastTapeToggle?.checked) {
    void instantLoadCurrentTape();
  } else {
    client.playTape();
  }
});
...
tapeEjectBtn?.addEventListener("click", async () => {
  client.ejectTape();
  if (mediaFileText) mediaFileText.textContent = "No tape loaded";
  tapeLoaded = false;
  updateTapePlayerAvailability();
...
tapeLibrarySearch.addEventListener("search", () => {
  libraryFilterText = tapeLibrarySearch.value;
  void renderLibrary();
});
tapeLibrarySearch.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    if (tapeLibrarySearch.value) {
      tapeLibrarySearch.value = "";
      libraryFilterText = "";
      void renderLibrary();
    }
    tapeLibrarySearch.blur();
  }
});
tapeLibraryFormatFilter.addEventListener("change", () => {
  libraryFilter = tapeLibraryFormatFilter.value as LibraryFilter;
  void renderLibrary();
});
...
confirmLoadPlay.addEventListener("click", () => void loadTapeIntoPlayer());
