...
// Pure register/flag arithmetic — no memory access, no T-state accounting (that's
// the caller's job via cpu.bus/cpu.contend at the correct point in each opcode's
// bus-cycle sequence). Every function here either mutates cpu.regs.A + cpu.f
// directly (the accumulator-based ops) or returns a new byte/word value leaving the
// caller to store it (the generic 8/16-bit ops shared by register- and
// memory-operand opcode variants).
...
export function cp8(cpu: Z80, value: number): void {
  ...
  f |= value & (Flag.F5 | Flag.F3); // undocumented: 5/3 come from the operand
  cpu.f = f;
}
...
/** ADD HL,rr / ADD IX,rr / ADD IY,rr — affects only H, N(=0), C and undocumented
 * 5/3 (from result high byte); S, Z, P/V are left untouched. */
...
/** ADC HL,rr (ED-prefixed) — full 16-bit flag set. */
...
/** SBC HL,rr (ED-prefixed) — full 16-bit flag set. */
...
// ---- accumulator-only rotates (RLCA/RRCA/RLA/RRA): don't touch S/Z/P/V ----------
...
// ---- generic 8-bit rotate/shift group (CB xx and DD/FD CB xx) -------------------
// These set the full S/Z/5/H(=0)/3/P/N(=0)/C flag set from the result.
...
/** Undocumented: shifts left, forcing bit 0 to 1 (vs. SLA which forces it to 0). */
...
/** BIT n,r — undocumented 5/3 flags come from the tested value itself. */
...
/** BIT n,(HL) / BIT n,(IX+d) / BIT n,(IY+d) — undocumented 5/3 flags come from the
 * high byte of MEMPTR instead of the tested value (a well-known Z80 quirk). */
