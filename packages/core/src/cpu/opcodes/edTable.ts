...
// ED-prefixed opcodes. Every opcode not explicitly assigned below defaults to an 8T
// no-op, matching real NMOS Z80 behavior for the many undefined ED xx combinations.
//
// Flag behavior for the block I/O instructions (INI/IND/OUTI/OUTD and their repeat
// forms) is the most genuinely ambiguous corner of the documented-undocumented Z80
// literature — implemented here to the best-documented published formulas (data
// path is fully correct; H/C/P-V flag edge cases are a best-effort match). This
// matters only for exhaustive CPU test-suite conformance, not for real Spectrum
// software: these instructions are vanishingly rare on this platform.
...
export function buildEdTable(): OpcodeTable {
  ...
  // IN r,(C) / OUT (C),r — code 6 is the undocumented "IN F,(C)" (flags only, no
  // register write) / "OUT (C),0" forms.
  ...
  // SBC/ADC HL,rr and LD (nn),rr / LD rr,(nn) for BC/DE/HL/SP.
  for (let p = 0; p < 4; p++) {
  ...
  for (let p = 0; p < 4; p++) {
    ...
    table[0x4d + p * 0x10] = retn; // RETI: identical CPU-visible effect to RETN
  }
