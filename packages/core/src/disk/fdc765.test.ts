...
describe("Fdc765 controller", () => {
  it("executes Recalibrate and Sense Interrupt Status", () => {
    ...
    expect(msr & MSR_DIO).not.toBe(0); // FDC has result data for CPU

    const st0 = fdc.readData();
    const pcn = fdc.readData();
    expect(st0 & 0x20).toBe(0x20); // Seek End
    expect(pcn).toBe(0);
  });
  ...
  it("reads sector data via Read Data command (0x06)", () => {
    ...
    // Read Data: [0x06, unit/head, C, H, R, N, EOT, GPL, DTL]
    ...
    // Now in result phase: 7 bytes [ST0, ST1, ST2, C, H, R, N]
    const st0 = fdc.readData();
    ...
    expect(st0 & 0xc0).toBe(0); // Normal termination
    expect(st1).toBe(0);
    expect(st2).toBe(0);
    expect(c).toBe(0);
    expect(h).toBe(0);
    expect(r).toBe(0xc2); // Incremented
    expect(n).toBe(2);
  });
  ...
  it("writes sector data via Write Data command (0x05)", () => {
    ...
    // Write Data: [0x05, unit/head, C, H, R, N, EOT, GPL, DTL]
    const cmd = [0x05, 0x00, 0, 0, 0xc1, 2, 0xc1, 0x2a, 0xff];
    for (const b of cmd) {
      fdc.writeData(b);
    }

    // Execution phase: write 512 bytes
    for (let i = 0; i < 512; i++) {
      fdc.writeData(0x42);
    }
