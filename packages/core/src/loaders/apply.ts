...
export function applySnapshotTo128k(
  ...
  if ("banks" in snapshot && snapshot.banks) {
    ...
    // .sna 128k: banks 5/2/[paged] are folded into `ram`; the rest are explicit.
    ...
    // 48K snapshot (.sna 48k or .z80 v1) applied to 128K machine: standard 48K RAM banks 5, 2, 0.
    machine.memory.pokeBank(5, snapshot.ram.subarray(0x0000, 0x4000));
    machine.memory.pokeBank(2, snapshot.ram.subarray(0x4000, 0x8000));
    machine.memory.pokeBank(0, snapshot.ram.subarray(0x8000, 0xc000));
  }
