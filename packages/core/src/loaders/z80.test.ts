...
import { parseZ80, writeZ80, writeZ80128k, writeZ8048k, writeZ80Plus3 } from "./z80.js";
...
describe("parseZ80", () => {
  it("parses a v1 header with uncompressed RAM", () => {
    const header = baseHeaderV1(0x8000, (3 << 1) | 0x00);
  ...
  it("parses a v2 header (additional length 23) with a single uncompressed page", () => {
    ...
    const pageHeader = Uint8Array.from([0xff, 0xff, 8]);
    ...
    expect(snapshot.ram[0]).toBe(0x77);
  });
  ...
  it("round-trips Machine128k through writeZ80128k and parseZ80", () => {
    ...
    machine.memory.writePagingRegister(0x13);
  ...
  it("round-trips MachinePlus3 through writeZ80Plus3 and parseZ80", () => {
    ...
    machine.memory.writePort1ffd(0x08);
  ...
  it("dispatches Machine48k through writeZ80/applySnapshot without touching AY state", () => {
    const machine = new Machine48k();
    machine.reset();
    machine.cpu.regs.pc = 0x8123;
    machine.memory.write8(0x8000, 0x34);

    const bytes = writeZ80(machine, 4);
    const parsed = parseZ80(bytes);
    expect(parsed.hardwareMode).toBe("48k");

    const loadedInto = new Machine48k();
    applySnapshot(loadedInto, parsed);
    expect(loadedInto.cpu.regs.pc).toBe(0x8123);
    expect(loadedInto.memory.read8(0x8000)).toBe(0x34);
  });
});
