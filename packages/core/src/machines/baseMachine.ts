import type { AyChip } from "../audio/ayChip.js";
...
export const ROM_TRAP_LD_BYTES = 0x0556;
export const ROM_TRAP_LD_SEARCH = 0x0569;
export const ROM_RETURN_SA_ALL = 0x053f;
...
export abstract class BaseMachine<M extends MemoryDevice = MemoryDevice> implements Z80Bus {
  ...
  protected executeTapeTrap(): boolean {
    ...
    const isAt569 = this.cpu.regs.pc === ROM_TRAP_LD_SEARCH;
    const expectedFlag = isAt569
      ? this.cpu.regs.bytes[RegIndex.A_]!
      : this.cpu.regs.bytes[RegIndex.A]!;
    const isLoad = isAt569
      ? (this.cpu.regs.bytes[RegIndex.F_]! & 1) !== 0
      : this.cpu.getFlag(Flag.C);
    ...
    const returnExit = (success: boolean): void => {
      ...
      if (isAt569) {
        ...
        this.cpu.regs.pc = ROM_RETURN_SA_ALL;
      }
    };
  ...
  protected mixAudio(
    ...
    const tapeAudio =
      this.tapeSoundEnabled && !this.fastTapeLoad && this.tape.isPlaying()
