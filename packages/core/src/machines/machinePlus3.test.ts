...
describe("MachinePlus3", () => {
  it("boots with real +3 ROMs without crashing or spinning at reset", () => {
    ...
    // Run for 120 frames (~2.4 seconds emulated time)
    for (let frame = 0; frame < 120; frame++) {
      machine.runFrame();
    }

    // Machine should have advanced past PC=0
    expect(machine.cpu.regs.pc).toBeGreaterThan(0);

    // Framebuffer should have rendered valid pixels
  ...
  it("dispatches port reads and writes for AY, paging, and FDC", () => {
    ...
    // Write 0x1FFD to turn on disk motor
    ...
    expect(msr & 0x80).toBe(0x80); // RQM set

    // Select and write AY register
    machine.writePort(0xfffd, 0x06); // R6 (noise period)
